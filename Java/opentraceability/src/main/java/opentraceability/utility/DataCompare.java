...
public class DataCompare {
    ...
    private static XElement FindMatchingNode(XElement xchild1, XElement x2, int i) throws Exception
    {
        ...
        else
        {
            ...
            String val = xchild1.getValue();
            String tag = xchild1.getTagName();
            if (!StringHelper.isNullOrEmpty(xsiType) && !StringHelper.isNullOrEmpty(val) && !StringHelper.isNullOrEmpty(tag))
            {
                xchild2 = ListExtensions.FirstOrDefault(x2.Elements().stream().filter(x -> x.Attribute("xsi:type").equals(xsiType)
                                                                                        && x.getValue().equals(val)
                                                                                        && x.getTagName().equals(tag)));
            }
