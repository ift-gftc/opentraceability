...
{
    public class EPCISTestGDSTServerClient
    {
        string _baseURL;
        EPCISDataFormat _format;
        EPCISVersion _version;

        public EPCISTestGDSTServerClient(string baseURL, EPCISDataFormat format, EPCISVersion version)
        {
            _baseURL = baseURL;
            _version = version;
            _format = format;
        ...
        public async Task ResolveGDSTMasterData(string blob_id, EPCISBaseDocument doc)
        {
            using (var clientItem = HttpClientPool.GetClient())
            {
                var client = clientItem.Value;
